...
require 'puppet/util/keycloak_validator'

# This file contains a provider for the resource type `keycloak_conn_validator`,
# which validates the keycloak connection by attempting an https connection.

Puppet::Type.type(:keycloak_conn_validator).provide(:puppet_https) do
  desc "A provider for the resource type `keycloak_conn_validator`,
        which validates the keycloak connection by attempting an http(s)
        connection to the keycloak server.  Uses the puppet SSL certificate
  ...
  def exists?
    ...
    unless success
      Puppet.notice("Failed to connect to keycloak within timeout window of #{timeout} seconds; giving up.")
    end

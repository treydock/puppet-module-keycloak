...
describe 'keycloak::client-template define:' do
  context 'creates client-template' do
    ...
    it 'should have created a client template' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/openid-connect-clients -r test' do
    ...
    it 'should have created protocol mapper email' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/openid-connect-clients/protocol-mappers/models -r test' do
        data = JSON.parse(stdout)
    ...
    it 'should have created protocol mapper family name' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/openid-connect-clients/protocol-mappers/models -r test' do
        ...
        expect(mapper['consentText']).to eq('${familyName}')
  ...
  context 'updates client-template' do
    ...
    it 'should have updated a client template' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/openid-connect-clients -r test' do
        data = JSON.parse(stdout)
        expect(data['name']).to eq('openid-connect-clients')
        expect(data['protocol']).to eq('openid-connect')
        expect(data['fullScopeAllowed']).to eq(false)
      end
    end
  end
  ...
  context 'creates saml client-template' do
    ...
    it 'should have created a client template' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/saml-clients -r test' do
        data = JSON.parse(stdout)
    ...
    it 'should have created protocol mapper username' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/saml-clients/protocol-mappers/models -r test' do
        data = JSON.parse(stdout)
        mapper = data.select { |d| d['name'] == 'username' }[0]
    ...
    it 'should have created protocol mapper role list' do
      on hosts, '/opt/keycloak/bin/kcadm-wrapper.sh get client-templates/saml-clients/protocol-mappers/models -r test' do
        ...
        expect(mapper['consentRequired']).to eq(false)

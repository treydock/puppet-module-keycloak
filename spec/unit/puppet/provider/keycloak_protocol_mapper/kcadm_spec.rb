...
describe Puppet::Type.type(:keycloak_protocol_mapper).provider(:kcadm) do
  ...
  describe 'self.instances' do
    it 'should create instances' do
      ...
      allow(@provider).to receive(:kcadm).with('get', 'client-scopes', 'master', nil, ['id']).and_return('[]')
      allow(@provider).to receive(:kcadm).with('get', 'client-scopes', 'test', nil, ['id']).and_return('[ { "id" : "oidc-clients" } ]')
      allow(@provider).to receive(:kcadm).with('get', 'client-scopes/oidc-clients/protocol-mappers/models', 'test').and_return(my_fixture_read('get-test.out'))
